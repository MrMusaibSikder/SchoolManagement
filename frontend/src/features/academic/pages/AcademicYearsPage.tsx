...
import { useAcademicYears, useCreateAcademicYear, useDeleteAcademicYear, useUpdateAcademicYear } from "../hooks/useAcademicData";
import type { AcademicYearDto } from "../types/academic.types";

export function AcademicYearsPage() {
  const { data = [], isPending, isError } = useAcademicYears();
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (editingId) {
      await updateYear.mutateAsync({ id: editingId, payload: { ...payload, id: editingId } });
    } else {
  ...
  function startEdit(item: AcademicYearDto) {
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Create and maintain academic years and mark the current one.</p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <Card>
          <CardHeader>
            <CardTitle>{editingId ? "Edit academic year" : "Create academic year"}</CardTitle>
            <CardDescription>Form validation is handled on the client before submission.</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-2">
                <Button type="submit" disabled={createYear.isPending || updateYear.isPending}>
                  {createYear.isPending || updateYear.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Plus className="mr-2 h-4 w-4" />}
        ...
        <Card>
          <CardHeader>
            <CardTitle>Years list</CardTitle>
            <CardDescription>Search and manage the available academic years.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="relative">
              <Search aria-hidden="true" className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
              <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search years" className="w-full rounded-md border bg-background py-2 pl-9 pr-3" />
            </div>
            {isPending ? <div className="flex items-center justify-center py-8 text-sm text-muted-foreground"><Loader2 className="mr-2 h-4 w-4 animate-spin" />Loading years…</div> : null}
            {isError ? <div className="rounded-lg border border-destructive/20 bg-destructive/5 p-4 text-sm text-destructive">Unable to load academic years.</div> : null}
            {!isPending && !isError && filtered.length === 0 ? <div className="rounded-lg border border-dashed p-6 text-sm text-muted-foreground">No academic years found.</div> : null}
              ...
              {filtered.map((item) => (
                <div key={item.id} className="flex items-center justify-between rounded-lg border p-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button type="button" variant="outline" className="h-9 w-9 p-0" onClick={() => deleteYear.mutate(item.id)}><Trash2 className="h-4 w-4" /></Button>
                  </div>
                </div>
              ))}
            </div>
